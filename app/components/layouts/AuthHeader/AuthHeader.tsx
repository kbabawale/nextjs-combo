...
const SearchResultDelivery = () => {
    return (
        <div className={`my-2 px-2 py-1 w-100 d-flex align-items-center justify-content-between ${styles.highlight}`}>
            ...
            <span className={`link hover color-safety-blue text-decoration-underline fw-700`}>View details</span>
        </div>
    )
}
    ...
    return (
        ...
        <header className={`w-100 px-5 py-3 d-flex align-items-center justify-content-between ${styles.header}`}>
            ...
            <div className={`${styles.search} flex-fill mx-5 py-2 px-4`}>
                ...
                <input type="search" onBlur={blurFn} onFocus={focusFn} ref={searchRef} onChange={searchFn} placeholder='Search driver names, pick-up codes or order numbers' />
                <div className={`${search ? 'd-flex' : 'd-none'} ${styles.suggestions} w-100 flex-column align-items-center border`}>
